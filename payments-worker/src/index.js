/* =========================================================
   ...
   Routes:
     ...
     - GET     /                  (health)
     - GET     /health            (health)
     - POST    /v1/pay/token      (internal-only)
     - POST    /v1/pay/verify     (KV-first, fallback to signature verify)
   ...
   KV (binding):
     - PAY_SESSIONS_KV
       key:
         tok:<signature>          → payload (JSON, large OK)
         sid:<session_id>         → full token (string)

   ENV (vars):
     - ALLOWED_ORIGINS
     - WEB_BASE_URL
     - PAY_SESSIONS_TTL_DAYS
     - TELEGRAM_CHAT_ID
     - TG_THREAD_CONFIRM
     - POINTS_RATE
     - AIRTABLE_BASE_ID
...
try {
  ...
  verifyTurnstile = mod.verifyTurnstile || null;
} catch (_) {
...
/* --------------------------------------------------------
   KV helpers — KEY = token signature (short)
-------------------------------------------------------- */
...
function requireKv(env) {
  if (!env.PAY_SESSIONS_KV) {
    throw new HttpError(500, { ok: false, error: "missing_kv_binding", detail: "PAY_SESSIONS_KV" });
  }
}
...
async function kvPutTokenPayload(token, payload, env) {
  ...
  if (!sig) throw new HttpError(400, { ok: false, error: "invalid_token_format" });

  // ✅ store payload by signature (short key)
  await env.PAY_SESSIONS_KV.put(`tok:${sig}`, JSON.stringify(payload), { expirationTtl: ttl });

  // ✅ optional: store full token by session_id (value can be long, key stays short)
...
async function kvGetPayloadByToken(token, env) {
  ...
  const raw = await env.PAY_SESSIONS_KV.get(`tok:${sig}`);
  if (!raw) return null;
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (path === "/v1/pay/token" && method === "POST") {
        // internal-only (X-Confirm-Key checked inside lib)
        ...
        const payload = {
          ...
          exp_ms: Date.now() + ttl * 1000,
        };

        const token = await signPayloadToToken(payload, env);

        // KV mapping required (confirm must not break)
        const kv = await kvPutTokenPayload(token, payload, env);

        const base = toStr(env.WEB_BASE_URL) || "https://mmdprive.webflow.io";
        const confirm_payment_url = `${base}/confirm/payment-confirmation?token=${encodeURIComponent(token)}`;
      ...
      if (path === "/v1/pay/verify" && method === "POST") {
        ...
        if (!token) return json({ ok: false, error: "token_required" }, 400, corsHeaders(cors));

        // 1) KV-first (tokens from /v1/pay/token must hit here)
        ...
        // 2) fallback (support older tokens)
