...
export function generateUsername(input: InviteIdentityInput): string {
  ...
  const nicknameBase = normalizeNicknameBase(
    input.nickname || input.mmd_client_name || input.client_name,
  ).slice(0, 24);
...
export function parseInviteIdentity(input: InviteIdentityInput): InviteIdentity {
  ...
  const nickname =
    toStr(input.nickname) || normalizeNicknameBase(parts[0] || input.client_name);
  ...
  const mmd_client_name = toStr(input.mmd_client_name || input.client_name) || nickname;
...
function buildAbsoluteUrl(baseUrl: string, input: string | undefined, fallbackPath: string): string {
